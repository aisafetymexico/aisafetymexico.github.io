// Dark-subtle screenshot script — 12 routes at 1440px against preview port 5081.
// Run with: node playwright/dark-subtle-shoot.mjs
import { chromium } from 'playwright';
import fs from 'node:fs/promises';
import path from 'node:path';

const BASE_URL = process.env.BASE_URL || 'http://127.0.0.1:5081';
const OUT_DIR = path.resolve('playwright/dark-subtle');
...
async function main() {
  ...
  for (const route of routes) {
    ...
    try {
      ...
      // Above-fold (viewport) — primary deliverable
      await page.screenshot({
        path: path.join(OUT_DIR, `${route.slug}-1440.png`),
        fullPage: false,
      });
      // Full-page (auxiliary, for context)
      await page.screenshot({
